...
if command -v dnf >/dev/null; then
    dnf install -y python3-pip python3-devel python3-venv git socat audit \
        policycoreutils-python-utils checkpolicy libselinux-utils 2>/dev/null || true
elif command -v yum >/dev/null; then
    yum install -y python3-pip python3-devel python3-venv git socat audit \
        policycoreutils-python-utils checkpolicy libselinux-utils 2>/dev/null || true
elif command -v apt-get >/dev/null; then
    apt-get update
    apt-get install -y python3-pip python3-dev python3-venv git socat auditd \
        selinux-utils policycoreutils checkpolicy 2>/dev/null || true
else
...
# 可选: 安装推理层依赖（LLM 推理需要）
"$VENV_DIR/bin/pip" install -e "$INSTALL_DIR[reasoner]" 2>/dev/null || \
    warn "推理层依赖安装失败（openai/anthropic SDK），本地模型仍可用"
...
# 推理层 API key（部署后请修改）
# 支持 deepseek / qwen / claude / ollama
# GCODE_REASONER_API_KEY=sk-xxx
ENVEOF
    chown "$GCODE_USER:$GCODE_GROUP" "$INSTALL_DIR/.env"
...
print('模型下载完成')
" && chown -R "$GCODE_USER:$GCODE_GROUP" "$INSTALL_DIR/models" || \
        warn "模型下载失败，首次启动时会自动下载（可能需要 HuggingFace 镜像: export HF_ENDPOINT=https://hf-mirror.com）"
else
    info "跳过模型预下载"
fi
...
if [[ "$SKIP_SELINUX" == false ]]; then
    if command -v getenforce &>/dev/null && [[ "$(getenforce)" != "Disabled" ]]; then
        ...
        if command -v checkmodule &>/dev/null && command -v semodule_package &>/dev/null; then
            ...
            info "SELinux 策略已安装" || \
            warn "SELinux 策略编译失败，请手动处理（参考 deploy/gcode-selinux.te）"
        else
            warn "缺少 checkmodule/semodule_package，跳过 SELinux 策略编译"
        fi
        rm -rf "$SELINUX_DIR"

        # 设置 socket 目录的 SELinux 上下文
        if command -v semanage &>/dev/null; then
            semanage fcontext -a -t svirt_socket_t "$RUN_DIR(/.*)?" 2>/dev/null || true
        fi
...
systemctl enable gcode-mcp-server.service 2>/dev/null || true
systemctl start gcode-mcp-server.service 2>/dev/null || true
sleep 1
systemctl enable gcode-security-guard.service 2>/dev/null || true
systemctl start gcode-security-guard.service 2>/dev/null || true

# ─── 验证 ─────────────────────────────────────────────────────
sleep 2
GUARD_STATUS=$(systemctl is-active gcode-security-guard.service 2>/dev/null || echo "inactive")
MCP_STATUS=$(systemctl is-active gcode-mcp-server.service 2>/dev/null || echo "inactive")
...
if [[ "$GUARD_STATUS" != "active" || "$MCP_STATUS" != "active" ]]; then
    echo ""
    warn "部分服务未正常启动，查看日志："
    echo "    journalctl -u gcode-security-guard -n 20"
    echo "    journalctl -u gcode-mcp-server -n 20"
fi
